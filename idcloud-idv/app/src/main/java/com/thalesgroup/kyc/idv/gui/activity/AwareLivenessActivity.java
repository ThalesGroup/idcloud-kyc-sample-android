...
import com.thalesgroup.kyc.idv.gui.fragment.AwareLivenessFragment;
import com.thalesgroup.kyc.idv.helpers.DataContainer;
import com.thalesgroup.kyc.idv.helpers.util.ImageUtil;

import java.lang.ref.WeakReference;

import static com.thalesgroup.kyc.idv.gui.MainActivity.CAPTURE_RETURN_CODE_ERR;
import static com.thalesgroup.kyc.idv.gui.MainActivity.CAPTURE_RETURN_CODE_OK;
...
public class AwareLivenessActivity extends ActivityWithSpinner implements FaceLiveness.LivenessActivityPresenter{
    ...
    public void onCaptureEnd() {
        try {
            ...
            final Bitmap bitmap = BitmapFactory.decodeByteArray(mLivenessApi.getCapturedImage(), 0, mLivenessApi.getCapturedImage().length);
            DataContainer.instance().mSelfie = ImageUtil.bitmapToBytes(bitmap, Bitmap.CompressFormat.JPEG);
        ...
        AwareLivenessActivity.this.setResult(CAPTURE_RETURN_CODE_OK, intent);
        AwareLivenessActivity.this.finish();
    }

    public void onCaptureTimedout() {
        Intent intent = new Intent();
        AwareLivenessActivity.this.setResult(CAPTURE_RETURN_CODE_ERR, intent);
        AwareLivenessActivity.this.finish();
    }
